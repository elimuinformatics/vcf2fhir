...
def _valid_record(record):
    ...
    if(record.FILTER is not None and len(record.FILTER) != 0):
        invalid_record_logger.debug(
            "Reason: VCF FILTER does not equal 'PASS' or '.', Record: %s, considered sample: %s", record, record.samples[0].data)
        return False
    if(len(record.samples) == 0 or record.samples[0].gt_type is None):
        invalid_record_logger.debug(
            "Reason: VCF FORMAT.GT is null ('./.', '.|.', '.', etc), Record: %s, considered sample: %s", record, record.samples[0].data)
        return False
    if(len(record.ALT) != 1 or (record.ALT[0].type != 'SNV' and record.ALT[0].type != 'MNV')):
        invalid_record_logger.debug(
            "Reason: ALT is not a simple character string, comma-separated character string or '.' (Rows where ALT is a token (e.g. 'DEL') are excluded), Record: %s, considered sample: %s", record, record.samples[0].data)
        return False
    if record.CHROM == "M" and ((len(record.samples[0].gt_alleles) == 1 and record.samples[0].gt_alleles[0] == "0") or len(record.samples[0].gt_alleles) == 2):
        invalid_record_logger.debug(
            "Reason: Mitochondrial DNA with GT = 0 or its diploid, Record: %s, considered sample: %s", record, record.samples[0].data)
        return False
    return True
...
def _get_fhir_json(vcf_reader, ref_build, patientID, has_tabix, conversion_region, region_studied, nocall_region, output_filename):
    ...
    general_logger.info(
        "Start adding Region studied observation followed by Variant observation corresponding to that region")
    if has_tabix == True:
                ...
                if prev_add_chrom != record.CHROM and (region_studied or nocall_region):
                    ...
                    while prev_add_chrom != record.CHROM:
                        current_ref_seq = _get_ref_seq_by_chrom(
                            ref_build, chrom)
                        _add_region_studied(
                            region_studied, nocall_region, fhir_helper, chrom, current_ref_seq, patientID)
